...
from ..data_utils import has_valid_conversational_example, maybe_apply_chat_template, maybe_extract_prompt
...
class DPOTrainer(BaseTrainer):
    ...
    def _prepare_dataset(
        ...
        with PartialState().main_process_first():
            # Drop invalid conversational examples (with None/missing role or content)
            if isinstance(dataset, Dataset):
                map_kwargs["desc"] = f"Filtering invalid conversational examples in {dataset_name} dataset"
                dataset = dataset.filter(
                    has_valid_conversational_example,
    ...
    def concatenated_forward(
        ...
        if self.args.length_normalize_loss and "ipo" not in self.loss_type and self.args.ld_alpha is None:
            token_counts = loss_mask.sum(-1)
            all_logps = (per_token_logps * loss_mask).sum(-1) / token_counts.clamp_min(1)
